...
export default function TipologiaConsegnaPage() {
    ...
    type TipologiaConsegnaFields = {
        tipoConsegna: TipologiaConsegna
    }

    const { tipoConsegna } = useAppSelector((state) => state.cart);
    ...
    const onSubmit = (data: TipologiaConsegnaFields) => {
        storeDispatch(pushTipologiaConsegna(data.tipoConsegna));
        if (data.tipoConsegna === "ASPORTO") {
                ...
                <div className="flex flex-col flex-grow space-y-4">
                    <div className="w-full md:w-1/2">
                        <form className="w-full m-0 flex flex-col space-y-2" onSubmit={handleSubmit(onSubmit)}>
                            ...
                            <select {...register("tipoConsegna")}
                                className={errors.tipoConsegna ? "text-input-invalid" : "text-input"}>
                                <option value="ASPORTO">Asporto</option>
                                <option value="DOMICILIO">A domicilio</option>
                            </select>
                            <div className="invalid-feedback">
                                {errors.tipoConsegna?.message}
                            </div>
