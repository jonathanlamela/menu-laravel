    ...
    <div class="flex flex-col p-8 flex-grow">
        ...
        <div class="flex flex-col flex-grow">
            <div class="w-full bg-slate-500 p-4">
                <p class="text-xl antialiased text-white">Cibi</p>
            </div>
            <div class="flex w-full bg-gray-100 p-2">
                <div class="w-1/2">
                    <div class="flex">
                        <a class="btn-primary" href="{{ route('admin.food.create') }}">Crea</a>
                    </div>
                </div>
                <div class="w-1/2 flex justify-end">
                    <form class="m-0">
                        <div class="flex flex-row space-x-2">
                            <input type="text" class="text-input bg-white" name="search" placeHolder="Cerca un cibo"
                ...
                <table class="w-full flex flex-col flex-grow">
                    <thead>
                        <tr class="h-10 flex flex-row items-center">
                            ...
                            <th class="w-5/12 text-left">Nome</th>
                            <th class="w-1/12 text-center hidden lg:block">Categoria</th>
                            <th class="w-1/12 text-center hidden lg:block">Prezzo</th>
                            <th class="w-4/12 text-center">Azioni</th>
                        </tr>
                    </thead>
                    <tbody>
                        @foreach ($items as $item)
                            <tr class="h-10 w-full odd:bg-gray-100 flex-row flex flex-grow">
                                ...
                                <td class="w-5/12 text-left flex items-center">{{ $item->name }}</td>
                                <td class="w-1/12 text-center hidden lg:flex items-center justify-center">
                                    {{ $item->category->name }}
                                </td>
                                <td class="w-1/12 text-center hidden lg:flex items-center justify-center">
                                ...
                                <td
                                    ...
                                    <a class="flex flex-row space-x-2 items-center justify-center p-2 hover:bg-green-700 hover:text-white"
                                        href="{{ route('admin.food.edit', ['food' => $item->id]) }}"><svg
                                    ...
                                    <a class="flex flex-row space-x-2 items-center justify-center p-2 hover:bg-red-700 hover:text-white"
                                        href="{{ route('admin.food.delete', ['food' => $item->id]) }}">
                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                            stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                                            <path stroke-linecap="round" stroke-linejoin="round"
