    ...
    <div class="flex flex-col p-8 flex-grow">
        ...
        <div class="flex flex-col flex-grow">
            <div class="w-full bg-slate-500 p-4">
                <p class="text-xl antialiased text-white">Categorie</p>
            </div>
            <div class="flex w-full bg-gray-100 p-2">
                <div class="w-1/2">
                    <div class="flex">

                        <a class="btn-primary" href="{{ route('admin.category.create') }}">Crea</a>
                    </div>
                </div>
                <div class="w-1/2
                                flex justify-end">
                ...
                <table class="w-full flex flex-col flex-grow">
                    <thead>
                        <tr class="h-10 flex flex-row items-center">
                            <th class="w-1/12 lg:w-1/12 text-center">#</th>
                            <th class="w-5/12 lg:w-8/12 text-left">Nome</th>
                    ...
                    <tbody>
                        @foreach ($items as $item)
                            <tr class="h-10 w-full odd:bg-gray-100 flex-row flex flex-grow">
                                <td class="w-1/12 lg:w-1/12 text-center flex items-center justify-center">
                                    {{ $item->id }}
                                </td>
                                <td class="w-5/12 lg:w-8/12 text-left flex items-center">{{ $item->name }}</td>
                                <td
                                    ...
                                    <a class="flex flex-row space-x-2 items-center justify-center p-2 hover:bg-green-700 hover:text-white"
                                        href="{{ route('admin.category.edit', ['category' => $item->id]) }}"><svg
                                    ...
                                    <a class="flex flex-row space-x-2 items-center justify-center p-2 hover:bg-red-700 hover:text-white"
                                        href="{{ route('admin.category.delete', ['category' => $item->id]) }}">
